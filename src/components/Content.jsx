    ...
    const displayPosts = posts.slice(pageVisited, pageVisited + postsPerPage).map((el, i) => {
        return (
            <div key={i} className='bg-white shadow-md max-w-md md:w-52 mx-auto my-3 md:mx-2 rounded-xl'>
                <p className='flex justify-between items-center px-5 md:px-2 py-3 text-lg md:text-sm truncate text-clip overflow-hidden'>Personnel ID: {el.id.value}
                    <span className='w-5'><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 md:h-4 md:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 12h.01M12 12h.01M19 12h.01M6 12a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0z" />
                    </svg></span></p>
                <div className='border-2'></div>
                <div className='h-60 md:h-full p-5 flex justify-items-start items-center xl:block'>
                    <div className=' rounded-full overflow-hidden border-2 xl:mx-auto md:w-32'>
                        <img src={el.picture.large} alt="personnel" className='object-cover object-center w-44 md:w-32' />
                    </div>
                    <div className='m-9 md:m-1 min-w-full'>
                        ...
                        <p className='text-lg-left md:text-sm font-normal hidden md:block'>{el.email}</p>
                    </div>
                </div>
            </div>
        )
